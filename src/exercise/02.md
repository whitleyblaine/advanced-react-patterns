...
The Compound Components Pattern enables you to provide a set of components that implicitly share state for a simple yet poweful declarative API for reusable components.

Compound components are components that work together to form a complete UI (like `<select>` and `<option>`).
With `<select>` and `<option>`, `<select>` manages the state of the UI, and `<option>` is essentially configuration for how the select should operate.

Start out with simple components (don't try to solve for every possible future usecase), but make it so your components are easily extensible.

The fundamental challenge with the type of API in this exercise is the state shared between components is implicit, which means the developer can't actually see or interact with the state or state updater mechanisms shared between the components. In this exercise we work to fix that. 

React.Children is a module that makes it easier to interact with the children prop. This is because the children prop can be any data type. The React.Children module allows us to deal with these different datatypes without worry about what they are. For more, see: https://www.netlify.com/blog/2020/12/17/react-children-the-misunderstood-prop/
