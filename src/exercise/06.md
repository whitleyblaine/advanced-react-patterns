...
Sometimes, people want to be able to manage the internal state of our component from the outside. The state reducer allows them to manage what state changes are made when a state change happens, but the CONTROL PROPS pattern allows them to make state changes themselves.

This is basically the same as controlled form elements in React.

You can destructure useRef:
```javascript
